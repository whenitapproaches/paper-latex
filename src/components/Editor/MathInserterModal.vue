...
const chooseImage = async (event, imageDataUrl = null) => {
	...
	const result = await store.dispatch("editor/insertMath", imageDataUrl)
	if (!result) {
	...
	if (result.hasError) {
		duration.value = 0
		base64Image.value = ""
		codeResult.value = $t("math_processing_error")
		return (isProcessing.value = false)
	}
	if (result.image?.base64) {
		base64Image.value = result.image.base64
	}
		...
		>
			...
			>
				<div class="bg-white dark:bg-zinc-700 px-4 pt-5 pb-4 sm:p-6 sm:pb-4">
					...
					<div
						class="flex mt-3 w-full text-zinc-500 dark:text-zinc-400 italic"
						v-show="duration"
					>
						<small class="ml-auto"
							>{{ $t("duration") }}: {{ duration }}ms</small
						>
					</div>
				</div>
				<div
